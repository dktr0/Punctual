...
tests = TestList [
  ...
  t "x <> y + z " (Right (Expression (Definition (Explicit "x") (After (Seconds 0.0)) DefaultCrossFade (Sum (FromTarget "y") (FromTarget "z"))) NoOutput)),
  t "x <> 0.5 + 0.4" (Right (Expression (Definition (Explicit "x") (After (Seconds 0.0)) DefaultCrossFade (Sum (Constant 0.5) (Constant 0.4))) NoOutput)),
  t "x <> -0.5 + -0.4" (Right (Expression (Definition (Explicit "x") (After (Seconds 0.0)) DefaultCrossFade (Sum (Constant (-0.5)) (Constant (-0.4)))) NoOutput)),
  t "x <> (-0.5) + (-0.4)" (Right (Expression (Definition (Explicit "x") (After (Seconds 0.0)) DefaultCrossFade (Sum (Constant (-0.5)) (Constant (-0.4)))) NoOutput)),
  t "x <> 0.5 - 0.4" (Right (Expression (Definition (Explicit "x") (After (Seconds 0.0)) DefaultCrossFade (Sum (Constant 0.5) (Product (Constant 0.4) (Constant (-1))))) NoOutput)),
