...
sampleWithDefault targetName d s t = maybe d id r
  where
    e = find ((==Explicit targetName) . target . definition) $ expressions s -- :: Maybe Expression
    g = fmap (graph  . definition) e
    r = fmap (sampleGraph (startTime s) t 0) g
